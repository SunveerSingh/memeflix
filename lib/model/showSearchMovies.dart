...
class ShowSearchMovieIMages extends StatefulWidget {
  String movieTitle;
  int movieId;
  ShowSearchMovieIMages(
      {Key key, @required this.movieTitle, @required this.movieId})
      : super(key: key);
  @override
  _ShowSearchMovieIMagesState createState() => _ShowSearchMovieIMagesState();
}

class _ShowSearchMovieIMagesState extends State<ShowSearchMovieIMages> {
  @override
  void initState() {
    super.initState();
  ...
  Future<List<dynamic>> fecthImages() async {
    final response = await http.get(
        Uri.parse(
            'https://api.themoviedb.org/3/movie/${widget.movieId}/images?api_key=f4ae53d60330375a486028e73667c90c'),
        headers: {
          HttpHeaders.authorizationHeader: "f4ae53d60330375a486028e73667c90c"
        });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("${widget.movieTitle} Images"),
